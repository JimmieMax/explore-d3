...
d3__WEBPACK_IMPORTED_MODULE_0__["select"]("#simple-chart").select("figure").append("svg").attr("width", "100%").attr("height", 120).selectAll("rect").data([250, 210, 170, 130, 90]).enter().append("rect").attr("x", 20).attr("y", (d, i) => i * simpleChart.rectHeight).attr("width", d => d).attr("height", simpleChart.rectHeight - 2).attr("fill", "orange");

// Scale
const scale = {};
scale.dataset = [1.2, 2.3, 0.9, 1.5, 3.3];
scale.rectHeight = 25;
scale.linear = d3__WEBPACK_IMPORTED_MODULE_0__["scaleLinear"]().domain([0, d3__WEBPACK_IMPORTED_MODULE_0__["max"](scale.dataset)]).range([0, 300]);

d3__WEBPACK_IMPORTED_MODULE_0__["select"]("#scale").select("figure").append("svg").attr("width", "100%").attr("height", 120).selectAll("rect").data(scale.dataset).enter().append("rect").attr("x", 20).attr("y", (d, i) => i * scale.rectHeight).attr("width", d => scale.linear(d)).attr("height", scale.rectHeight - 2).attr("fill", "orange");

// Axis
const axis = {};
axis.dataset = [2.5, 2.1, 1.7, 1.3, 0.9];
axis.rectHeight = 25;
axis.linear = d3__WEBPACK_IMPORTED_MODULE_0__["scaleLinear"]().domain([0, d3__WEBPACK_IMPORTED_MODULE_0__["max"](axis.dataset)]).range([0, 300]);
axis.axis = d3__WEBPACK_IMPORTED_MODULE_0__["axisBottom"](axis.linear);
axis.svg = d3__WEBPACK_IMPORTED_MODULE_0__["select"]("#axis").select("figure").append("svg");

axis.svg.attr("width", "100%").attr("height", 200).selectAll("rect").data(axis.dataset).enter().append("rect").attr("x", 20).attr("y", (d, i) => i * axis.rectHeight + 20).attr("width", d => axis.linear(d)).attr("height", axis.rectHeight - 2).attr("fill", "orange");

axis.svg.append("g").attr("transform", "translate(20,150)").call(axis.axis);
